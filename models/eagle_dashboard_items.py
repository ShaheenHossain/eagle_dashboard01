...
class KsDashboardNinjaItems(models.Model):
    ...
    eagle_model_id = fields.Many2one('ir.model', string='Model', required=True,
                                  domain="[('access_ids','!=',False),('transient','=',False),('model','not ilike','base_import%'),('model','not ilike','ir.%'),('model','not ilike','web_editor.%'),('model','not ilike','web_tour.%'),('model','!=','mail.thread'),('model','not ilike','eagle_dash%')]")
    ...
    eagle_record_field_2 = fields.Many2one('ir.model.fields',
                                        domain="[('model_id','=',eagle_model_id_2),('name','!=','id'),('store','=',True),'|','|',('ttype','=','integer'),('ttype','=','float'),('ttype','=','monetary')]",
                                        string="Kpi Record Field")
    eagle_record_count_2 = fields.Float(string="KPI Record Count", readonly=True)
    ...
    eagle_record_field = fields.Many2one('ir.model.fields',
                                      domain="[('model_id','=',eagle_model_id),('name','!=','id'),('store','=',True),'|','|',('ttype','=','integer'),('ttype','=','float'),('ttype','=','monetary')]",
                                      string="Record Field")
    ...
    eagle_date_filter_field = fields.Many2one('ir.model.fields',
                                           domain="[('model_id','=',eagle_model_id),'|',('ttype','=','date'),('ttype','=','datetime')]",
    ...
    eagle_date_filter_field_2 = fields.Many2one('ir.model.fields',
                                             domain="[('model_id','=',eagle_model_id_2),'|',('ttype','=','date'),('ttype','=','datetime')]",
    ...
    eagle_list_view_fields = fields.Many2many('ir.model.fields', 'eagle_dn_list_field_rel', 'list_field_id', 'field_id',
                                           domain="[('model_id','=',eagle_model_id),('store','=',True),"
    ...
    def eagle_get_record_count(self):
        for rec in self:
            ...
            elif rec.eagle_record_count_type in ['sum', 'average'] and rec.eagle_record_field:
    ...
    def eagle_convert_into_proper_domain(self, eagle_domain, rec):
        ...
        if not rec.eagle_date_filter_selection or rec.eagle_date_filter_selection == "l_none":
            ...
            self.eagle_set_date_filter()
            selected_start_date = rec.eagle_item_start_date
            selected_end_date = rec.eagle_item_end_date

            if rec.eagle_item_start_date and rec.eagle_item_end_date and rec.eagle_date_filter_field.name:

                if rec.eagle_compare_period:

                    if rec.eagle_compare_period > 0:
                        selected_end_date = selected_end_date + (
                                selected_end_date - selected_start_date) * rec.eagle_compare_period
                    elif rec.eagle_compare_period < 0:
                        selected_start_date = selected_start_date + (
                                selected_end_date - selected_start_date) * rec.eagle_compare_period

                if rec.eagle_year_period and rec.eagle_year_period != 0:
        ...
        if eagle_date_domain:
            ...
            rec.eagle_isDateFilterApplied = True
    ...
    @api.multi
    @api.onchange('eagle_chart_relation_sub_groupby')
    def get_chart_sub_groupby_type(self):
        for rec in self:
            if rec.eagle_chart_relation_sub_groupby.ttype == 'datetime' or rec.eagle_chart_relation_sub_groupby.ttype == 'date':
    ...
    @api.depends('eagle_chart_measure_field', 'eagle_chart_relation_groupby', 'eagle_chart_date_groupby', 'eagle_domain',
                 ...
                 'eagle_compare_period', 'eagle_year_period')
    def eagle_get_chart_data(self):
        for rec in self:
            if not rec.eagle_chart_relation_groupby or rec.eagle_chart_groupby_type == "date_type" and not rec.eagle_chart_date_groupby:
                rec.eagle_chart_relation_sub_groupby = False
                rec.eagle_chart_date_sub_groupby = False

            if rec.eagle_dashboard_item_type and rec.eagle_dashboard_item_type != 'eagle_tile' and rec.eagle_dashboard_item_type != 'eagle_list_view' and rec.eagle_model_id and rec.eagle_chart_data_count_type:
                ...
                if ((rec.eagle_chart_data_count_type != "count" and eagle_chart_measure_field) or (
                        rec.eagle_chart_data_count_type == "count" and not eagle_chart_measure_field)) and not rec.eagle_chart_relation_sub_groupby:
                    if rec.eagle_chart_relation_groupby.ttype == 'date' and rec.eagle_chart_date_groupby in (
                        ...
                        eagle_chart_date_groupby = 'day'  # when date_type doesn't have time
                    else:
                        eagle_chart_date_groupby = rec.eagle_chart_date_groupby

                    if (
                            rec.eagle_chart_groupby_type == 'date_type' and rec.eagle_chart_date_groupby) or rec.eagle_chart_groupby_type != 'date_type':
                        ...
                        if rec.eagle_chart_groupby_type == 'date_type' and rec.eagle_goal_enable and rec.eagle_dashboard_item_type in [
                            ...
                            if selected_start_date and selected_end_date:
                                eagle_goal_domain.extend([('eagle_goal_date', '>=', selected_start_date.date()),
                                                       ('eagle_goal_date', '<=', selected_end_date.date())])
                            ...
                            if rec.eagle_goal_lines and len(rec.eagle_goal_lines) != 0:
                                ...
                                for label in eagle_chart_records:
                                    eagle_chart_data['domains'].append(domains.get(label, ['id', '>', 0]))
                                    counterr = 0
                            ...
                            if rec.eagle_goal_bar_line:
                                ...
                                eagle_chart_data['datasets'].append(eagle_goal_datasets)
                                                            ...
                                                            (
                                                                    rec.eagle_chart_sub_groupby_type == 'date_type' and rec.eagle_chart_date_sub_groupby) or
                                                            (rec.eagle_chart_sub_groupby_type == 'other')):
                    if rec.eagle_chart_relation_sub_groupby.ttype == 'date':
                        ...
                        eagle_chart_date_sub_groupby, eagle_chart_date_groupby = rec.eagle_chart_date_sub_groupby, rec.eagle_chart_date_groupby  # doesn't have time in date
                    else:
                        eagle_chart_date_sub_groupby, eagle_chart_date_groupby = rec.eagle_chart_date_sub_groupby, rec.eagle_chart_date_groupby
                    if len(eagle_chart_measure_field) != 0 or rec.eagle_chart_data_count_type == 'count':
                        ...
                        for res in eagle_chart_record:
                            ...
                            if res[eagle_chart_groupby_relation_fields[0]] and res[eagle_chart_groupby_relation_fields[1]]:
                                if rec.eagle_chart_groupby_type == 'date_type':
                                    # x-axis modification
                                    if rec.eagle_chart_date_groupby == "day" and rec.eagle_chart_date_sub_groupby in [
                                        "quarter", "year"]:
                                        label = " ".join(res[eagle_chart_groupby_relation_fields[0]].split(" ")[0:2])
                                    elif rec.eagle_chart_date_groupby in ["minute", "hour"] and rec.eagle_chart_date_sub_groupby in [
                                ...
                                if rec.eagle_chart_data_count_type != 'count':
                                    for ress in rec.eagle_chart_measure_field:
                                        ...
                                        value.append(res.get(
                                            ress.name) if rec.eagle_chart_data_count_type == 'sum' else res.get(
                        ...
                        if len(eagle_data) != 0:
                            ...
                            if rec.eagle_goal_enable and rec.eagle_standard_goal_value:
                                goal_dataset = []
                                length = len(eagle_chart_data['datasets'][0]['data'])
                                for i in range(length):
                                    goal_dataset.append(rec.eagle_standard_goal_value)
    ...
    @api.depends('eagle_domain', 'eagle_dashboard_item_type', 'eagle_model_id', 'eagle_sort_by_field', 'eagle_sort_by_order',
                 ...
                 'eagle_item_start_date', 'eagle_compare_period', 'eagle_year_period')
            ...
            if rec.eagle_list_view_type and rec.eagle_dashboard_item_type and rec.eagle_dashboard_item_type == 'eagle_list_view' and \
                ...
                if rec.eagle_list_view_type == "ungrouped":
                    ...
                    if rec.eagle_chart_groupby_type == 'relational_type':
                        ...
                        eagle_list_view_records = self.env[rec.eagle_model_name].read_group(eagle_chart_domain, eagle_list_field,
                                                                                      [
                                                                                          rec.eagle_chart_relation_groupby.name + ':' + rec.eagle_chart_date_groupby],
                                                                                      orderby=orderby, limit=limit)
                        for res in eagle_list_view_records:
    ...
    @api.multi
    @api.depends('eagle_dashboard_item_type', 'eagle_model_id', 'eagle_model_id_2', 'eagle_record_field', 'eagle_goal_enable',
                 'eagle_standard_goal_value', 'eagle_record_field_2', 'eagle_record_count_type_2', 'eagle_domain', 'eagle_domain_2',
                 'eagle_date_filter_selection', 'eagle_item_start_date', 'eagle_record_count_type',
                 'eagle_item_end_date', 'eagle_previous_period', 'eagle_item_start_date_2', 'eagle_item_end_date',
                 'eagle_compare_period', 'eagle_year_period')
    def eagle_get_kpi_data(self):
        for rec in self:
            if rec.eagle_dashboard_item_type and rec.eagle_dashboard_item_type == 'eagle_kpi' and rec.eagle_model_id:
                ...
                if rec.eagle_record_count_type == 'count':
                    eagle_record_count = rec.eagle_fetch_model_data(rec.eagle_model_name, rec.eagle_domain, 'search_count', rec)

                elif rec.eagle_record_count_type in ['sum', 'average'] and rec.eagle_record_field:
                    eagle_records_grouped_data = rec.eagle_fetch_model_data(rec.eagle_model_name, rec.eagle_domain, 'read_group',
                                                                      rec)
                    if eagle_records_grouped_data and len(eagle_records_grouped_data) > 0:
                        eagle_records_grouped_data = eagle_records_grouped_data[0]
                        if rec.eagle_record_count_type == 'sum' and eagle_records_grouped_data.get('__count', False) and (
                                eagle_records_grouped_data.get(rec.eagle_record_field.name)):
                            eagle_record_count = eagle_records_grouped_data.get(rec.eagle_record_field.name, 0)
                        elif rec.eagle_record_count_type == 'average' and eagle_records_grouped_data.get(
                                '__count', False) and (eagle_records_grouped_data.get(rec.eagle_record_field.name)):
                            eagle_record_count = eagle_records_grouped_data.get(rec.eagle_record_field.name,
                                                                          0) / eagle_records_grouped_data.get('__count', 1)
                        else:
                ...
                if rec.eagle_model_id_2 and rec.eagle_record_count_type_2:
                    eagle_kpi_data_model_2 = rec.eagle_get_model_2_data(rec.eagle_model_name_2, rec.eagle_record_count_type_2,
                                                                  rec.eagle_record_field_2, rec.eagle_domain_2, rec)
    ...
    def eagle_get_model_2_data(self, eagle_model_name_2, eagle_record_count_type_2, eagle_record_field_2, eagle_domain, rec):
        if rec.eagle_record_count_type_2 == 'count':
            eagle_record_count = rec.eagle_fetch_model_data_2(eagle_model_name_2, eagle_domain, 'search_count', rec)

        elif rec.eagle_record_count_type_2 in ['sum', 'average'] and eagle_record_field_2:
            eagle_records_grouped_data = rec.eagle_fetch_model_data_2(eagle_model_name_2, eagle_domain, 'read_group', rec)
            if eagle_records_grouped_data and len(eagle_records_grouped_data) > 0:
                eagle_records_grouped_data = eagle_records_grouped_data[0]
                if rec.eagle_record_count_type_2 == 'sum' and eagle_records_grouped_data.get('__count', False) and (
                        eagle_records_grouped_data.get(eagle_record_field_2.name)):
                    eagle_record_count = eagle_records_grouped_data.get(eagle_record_field_2.name, 0)
                elif rec.eagle_record_count_type_2 == 'average' and eagle_records_grouped_data.get(
                        '__count', False) and (eagle_records_grouped_data.get(eagle_record_field_2.name)):
                    eagle_record_count = eagle_records_grouped_data.get(eagle_record_field_2.name,
                                                                  0) / eagle_records_grouped_data.get('__count',
                                                                                                   1)
                else:
                    eagle_record_count = 0
        ...
        eagle_kpi_data_model_2[
            'record_field'] = 'count' if eagle_record_count_type_2 == 'count' else eagle_record_field_2.field_description
        eagle_kpi_data_model_2['record_data'] = eagle_record_count

        return eagle_kpi_data_model_2
    ...
    def eagle_convert_into_proper_domain_2(self, eagle_domain_2, rec):
        ...
        if (not rec.eagle_date_filter_selection_2) or rec.eagle_date_filter_selection_2 == "l_none":
            ...
            self.eagle_set_date_filter_2()
            selected_start_date = rec.eagle_item_start_date_2
            selected_end_date = rec.eagle_item_end_date_2
        if eagle_domain_2:
            # try:
            proper_domain = eval(eagle_domain_2)
            if selected_start_date and selected_end_date and rec.eagle_date_filter_field:
                proper_domain.extend([(rec.eagle_date_filter_field_2.name, ">=",
                ...
                rec.eagle_isDateFilterApplied = False
        else:
            if selected_start_date and selected_end_date and rec.eagle_date_filter_field:
                proper_domain = [(rec.eagle_date_filter_field_2.name, ">=",
                                  selected_start_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)),
                                 (rec.eagle_date_filter_field_2.name, "<=",
                                  selected_end_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT))]
            else:
    ...
    def eagle_fetch_chart_data(self, eagle_model_name, eagle_chart_domain, eagle_chart_measure_field, eagle_chart_measure_field_2,
        ...
        for res in eagle_chart_records:
            ...
            if res[eagle_chart_groupby_field] and all(
                    measure_field in res for measure_field in eagle_chart_measure_field):
                if eagle_chart_groupby_type == "relational_type":
    ...
    def eagle_fetch_drill_down_data(self, item_id, domain, sequence):
        ...
        orderby = record.eagle_sort_by_field.name if record.eagle_sort_by_field else "id"
        if record.eagle_sort_by_order:
            orderby = orderby + " " + record.eagle_sort_by_order
        limit = record.eagle_record_data_limit if record.eagle_record_data_limit and record.eagle_record_data_limit > 0 else False
                ...
                if field_type == 'date_type':
                    if eagle_chart_date_groupby in ['minute','hour'] :
                        if eagle_chart_date_sub_groupby in ["month","week","quarter", "year"]:
                            ...
                            data['value'].sort(key=lambda x: int(str(eagle_sorted_months.index(x['x'].split(" ")[2])+1) + x['x'].split(" ")[1] + x['x'].split(" ")[0].replace(":","")), reverse=reverse)
                        else :
