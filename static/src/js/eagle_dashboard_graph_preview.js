eagle.define('eagle_dashboard_list.eagle_dashboard_graph_preview', function (require) {
    "use strict";
    ...
    var KsGraphPreview = AbstractField.extend({
        ...
        init: function (parent, state, params) {
            ...
            this.jsLibs.push('/eagle_dashboard/static/lib/js/Chart.bundle.min.js',
        ...
        _render: function () {
            this.$el.empty()
            if (this.recordData.eagle_dashboard_item_type !== 'eagle_tile' && this.recordData.eagle_dashboard_item_type !== 'eagle_kpi' && this.recordData.eagle_dashboard_item_type !== 'eagle_list_view') {
                if(this.recordData.eagle_model_id){
                    if (this.recordData.eagle_chart_groupby_type == 'date_type' && !this.recordData.eagle_chart_date_groupby) {
                        return this.$el.append($('<div>').text("Select Group by date to create chart based on date groupby"));
                    }else if(this.recordData.eagle_chart_data_count_type==="count" && !this.recordData.eagle_chart_relation_groupby){
                        this.$el.append($('<div>').text("Select Group By to create chart view"));
                    }else if(this.recordData.eagle_chart_data_count_type!=="count" && (this.recordData.eagle_chart_measure_field.count === 0 || !this.recordData.eagle_chart_relation_groupby)){
                        this.$el.append($('<div>').text("Select Measure and Group By to create chart view"));
                    }else if(!this.recordData.eagle_chart_data_count_type){
        ...
        _getChartData: function () {
            ...
            var eagle_chart_name;
            if (field.name) eagle_chart_name = field.name;
            else if (field.eagle_model_name) eagle_chart_name = field.eagle_model_id.data.display_name;
            else eagle_chart_name = "Name";

            this.chart_type = this.recordData.eagle_dashboard_item_type.split('_')[1];
            this.chart_data = JSON.parse(this.recordData.eagle_chart_data);

            var $chartContainer = $(QWeb.render('eagle_chart_form_view_container', {
                eagle_chart_name: eagle_chart_name
            }));
        ...
        renderChart: function(){
            var self = this;
            if(this.recordData.eagle_chart_measure_field_2.count && this.recordData.eagle_dashboard_item_type === 'eagle_bar_chart'){
                var scales  = {}
                scales.yAxes = [
                    {
                        type: "linear",
            ...
            var chart_plugin = [];
            if (this.recordData.eagle_show_data_value) {
            ...
            if(this.chart_data && this.chart_data["datasets"].length>0){
               self.ksChartColors(this.recordData.eagle_chart_item_color, this.ksMyChart, this.chart_type, this.chart_family,this.recordData.eagle_show_data_value);
            }
        },

        ksChartColors: function (palette, ksMyChart, ksChartType, ksChartFamily,eagle_show_data_value) {
            ...
            if (ksChartFamily == "circle") {
                ...
                if(this.recordData.eagle_semi_circle_chart && (chartType === "pie" || chartType ==="doughnut" ) ){
                    ...
                    switch (ksChartType) {
                        ...
                        case "horizontalBar":
                            if (datasets[i].type && datasets[i].type=="line"){
                                ...
                                options.scales.xAxes[0].stacked = this.recordData.eagle_bar_chart_stacked;
                                options.scales.yAxes[0].stacked = this.recordData.eagle_bar_chart_stacked;
                            }
                            break;
                        case "line":
                            datasets[i].borderColor = chartColors[i];
    ...
    registry.add('eagle_dashboard_graph_preview', KsGraphPreview);
